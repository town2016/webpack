...
<script>
  const baseUrl = process.env.NODE_ENV === 'development' ? '/api_service/' : '/';
  ...
  export default {
    ...
    data() {
      return {
        picUrl: `${baseUrl}common.do?method=createCode&subtime=${Date.now()}`, // 图形验证码url
    ...
    methods: {
      ...
      refreshPicUrl() {
        this.picUrl = `${baseUrl}common.do?method=createCode&subtime=${Date.now()}`;
      },
    },
    watch: {
      value(val) {
