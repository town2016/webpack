    ...
    methods: {
      ...
      async pay() {
        try {
          ...
          // 筛选已被选中的还款
          let detail = this.repaymentDetail.filter(v => v.checked);
          // 还款类型 1：单期，2：多期，不需要判断等于0，没有选中还款内容无法弹窗
          let repayType = detail.length > 1 ? 2 : 1;
          ...
          let params = {
            ...
            repaymentListJson: JSON.stringify([
              {
                applyid: detail[0].oId,
                money: this.totalRepay,
                payType: 'creditLoan',
                phases: detail.map(v => v.phase).reduce((a,b) => a+','+b),
                repayType: repayType
              }
            ])
          }
