...
<template>
  <div id="messageAuth" class="container">
    ...
    <!--提交按钮-->
    <base-button text='提交' :type='submitButtonState' mt='.3rem' @click.native="submit"></base-button>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      async smsCodeBtnClick() {
        let status = await this.$parent.nextClick(true);
        if(status == 203){
          this.startCountDown();
        }
      },
      ...
      async submit() { // 提交操作
        ...
        await this.$emit('messageAuth',this.smsCode);
        this.$parent.nextClick(false);
      }
    },
    mounted() {
      this.startCountDown();
