...
const fiilterOrderData = function(arr, isPreFee) {
  ...
  return { shouldRepayArr: returnArr };
};

export default {
  // 处理还款订单列表
  [_BILL_DETAIL_MANAGE](_state, _payload) {
    const { result, oId } = _payload;
    const { notYet, loanData = null } = result;
    const { shouldRepayArr: shoulReturn } = fiilterOrderData(result.shoulReturn, false); // 过滤非前置单
    const realNotYet = [];
    _state.oId = oId;
    if (loanData) {
      const { shouldRepayArr } = fiilterOrderData([loanData], true); // 过滤前置单
      _state.shouldRepayArr = shouldRepayArr; // 过滤前置单
      _state.shouldRepayArr.isNotPrePay = false;
      _state.unpaidBillsArr = [];
    } else {
      ...
      shoulReturn.forEach((x) => {
        ...
        const earlyRepayFee = x.prepayTotFee * 1;
        const coustomManagerFee = coustomTotal - coustomInterest - coustomCapital - earlyRepayFee;
        x.overdueInterest = overdueInterest.toFixed(2); // 逾期
        x.isOverdueBill = x.repayStatus === 2; // 是否为逾期账单
        x.earlyRepayFee = earlyRepayFee.toFixed(2);
      ...
      notYet.forEach((x) => { // 过滤为0的订单
        if (x.shouldTotal !== 0) {
          realNotYet.push(x);
        }
      });

      _state.shouldRepayArr = shoulReturn;
      _state.unpaidBillsArr = realNotYet;
    }
  },
  // 还原模块数据
  [CLEAR_WHOLE_DATA](_state, _payload) {
    if (_payload === 'shouldRepay') {
      _state.shouldRepayArr = null;
    } else if (_payload === 'unpaid') {
      _state.unpaidBillsArr = null;
    } else {
      _state.shouldRepayArr = null;
      _state.unpaidBillsArr = null;
    }
  ...
  [STATE_BECOME_FAIL](_state, _payload) {
    if (_payload === 'shouldRepay') {
      ...
      _state.unpaidBillsArr = false;
    } else {
      _state.shouldRepayArr = false;
      _state.unpaidBillsArr = false;
    }
  }
  // [SUBMIT_BANK_INFO](state, payload) {
