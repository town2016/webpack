...
import {
  ...
  API_GET_BIND_BANK_CARD_HTTP_REQUEST,
...
} from '@/api/h5config/bankCardConfig';
import {
...
import {
  ...
  SET_CARD_AGREEMENT
} from '@/store/h5processModules/bankCard/enum/mutationsEnum';
import storage from "@/utils/storage";

export default {
  // 获取首张银行卡
  async [GET_FIRST_REPAY_CARD]({ commit, dispatch }, { isLoading = false, isToast = false } = {}) {
    try {
      ...
      storage.write("bankCardInfo",JSON.stringify(data.result));
  ...
  // 获取绑卡URL
  async [GET_BIND_BANK_CARD_HTTP_REQUEST]({ commit, dispatch }, { isLoading = true, isToast = false, callBackUrl = '' } = {}) {
    try {
      const data = await dispatch(FETCH_SERVER, {
        apiName: API_GET_BIND_BANK_CARD_HTTP_REQUEST,
        config: { isLoading, isToast },
        params: { callBackUrl },
        extraFetchConfig: {}
      }, { root: true });
